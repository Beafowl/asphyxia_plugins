  ...
  $.getJSON('static/asset/json/music_db.json', function (json) {
    try {
      ...
      if (xhr.status === 200) {
        ...
        if (custom && custom.mdb && custom.mdb.music)
          json.mdb.music = json.mdb.music.concat(custom.mdb.music);
      }
    } catch (e) {}
    ...
    for (let mdata in music_db.mdb.music) {
      ...
      if (music_db.mdb.music[mdata]['info']['inf_ver'] != 0) {
        temp_data.oth =
          music_db.mdb.music[mdata]['difficulty']['infinite'] +
          ' | ' +
          getInfDifficulty(music_db.mdb.music[mdata]['info']['inf_ver']);
      }
