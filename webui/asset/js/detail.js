...
function getSkillTitle() {
  ...
  if (sk.length <= 0 || sk[0].name === (undefined || 0)) return '';
  return skill_title_db.filter(e => e.id === sk[0].name)[0].name;
}
...
function getMedal(name, clear, version) {
  ...
  switch (clear) {
    ...
    case 5:
      return version === 6 ? 1.1 : 1.06;
    case 6:
      return version === 6 ? 1.04 : 1.1;
  }
}
...
function getAppealCard(appeal, version) {
  ...
  return (
    ...
    (result.length > 0 ? result[0]['info']['texture'] : defaultCard[version - 6]) +
    '.png'
  );
}
...
function singleScoreVolforce(score) {
  ...
  var tempVF =
    ...
    getGrade(false, score.grade) *
    getMedal(false, score.clear, score.version) *
    2;
  // console.log(tempVF);
  ...
  $('#statistic-table').append(
    $('<thead>').append(
      $('<tr>')
        ...
        .append($('<th>').append('EFFECTIVE CLEAR'))
        .append($('<th>').append('EXCESSIVE CLEAR'))
        .append($('<th>').append('MAXXIVE CLEAR'))
        .append($('<th>').append('UC'))
...
function preSetTableAvg(type) {
  ...
  $('#statistic-table').append(
    $('<thead>').append(
      $('<tr>').append($('<th>').append(type)).append($('<th>').append('Average Score'))
    )
  );
}
...
function setCMpD() {
  ...
  $('#statistic-table').fadeOut(200, function () {
    ...
    $('#statistic-table').append(baseTBodyCMpD).removeClass('is-narrow').css('width', '45%');
...
function setCMpL() {
  ...
  $('#statistic-table').fadeOut(200, function () {
    ...
    $('#statistic-table').append(baseTBodyCMpL).removeClass('is-narrow').css('width', '45%');
    $('#statistic-table').fadeIn(200);
  });
}
...
function setGpD() {
  ...
  $('#statistic-table').fadeOut(200, function () {
    ...
    $('#statistic-table').append(baseTBodyGpD).removeClass('is-narrow').css('width', '45%');
    $('#statistic-table').fadeIn(200);
  });
  //$('#statistic-table').empty();
}
...
function setGpL() {
  ...
  $('#statistic-table').fadeOut(200, function () {
    ...
    $('#statistic-table').append(baseTBodyGpL).removeClass('is-narrow').css('width', '45%');
    $('#statistic-table').fadeIn(200);
  });
  //$('#statistic-table').empty();
}
...
function setASpL() {
  ...
  $('#statistic-table').fadeOut(200, function () {
    ...
    $('#statistic-table').append(baseTBodyASpL).addClass('is-narrow').css('width', '30%');
    $('#statistic-table').fadeIn(200);
  });
  //$('#statistic-table').empty();
}
...
function setUpStatistics(profileVer) {
  ...
  score_db
    .filter(sc => sc.version === profileVer)
    .forEach(function (currentValue, index, array) {
      ...
      ASpLArray[parseInt(getSongLevel(currentValue.mid, currentValue.type)) - 1][1] +=
        currentValue.score;
    });
  ...
  for (var diff = 0; diff < 6; diff++) {
    baseTBodyCMpD.append(
      $('<tr>')
        ...
        .append($('<td>').append(CMpDArray[diff][5]))
    );
  }
  for (var lv = 1; lv <= 20; lv++) {
    baseTBodyCMpL.append(
      $('<tr>')
        ...
        .append($('<td>').append(CMpLArray[lv - 1][5]))
    );
  }
  for (var diff = 0; diff < 6; diff++) {
    baseTBodyGpD.append(
...
function getPlayerSkill() {
  ...
  let courseDataFil = course_data.filter(
    e =>
      ...
      e.cid === k[0].level &&
      (k[0].type !== undefined ? k[0].type : 0) === (e.stype !== undefined ? e.stype : 0)
  );
    ...
    $('.arena-details')
      ...
      .append(
        $('<div class="tile is-parent is-5" style="position:relative">').append(
          $('<article class="tile is-child arena-details-child-right">')
            .append(
            ...
            .append(
              sznData.megamixRate !== (0 || undefined)
                ? $('<p class="title" style="font-family: testfont">Ultimate Rate</p>').append(
                ...
                : $('<p class="title" style="font-family: testfont">Megamix Rate</p>').append(
                    $(
                      '<div class="content">' +
                        sznData.megamixRate +
                        ' <img style="width:20px" src="static/asset/arena_rank/mixstar.png"></p></div>'
                    )
                  )
            )
        )
...
$(document).ready(function () {
  ...
  currentVersion =
    urlParams.has('version') && urlParams.get('version') !== ''
      ...
      : profile_data[profile_data.length - 1].version;
  ...
  $.when(
          ...
          .append(
            $('<div class="card-header">').append(
              $('<p class="card-header-title">')
                .append($('<span class="icon">').append($('<i class="mdi mdi-account-edit">')))
                .append('Basic Data')
            )
          )
          .append(
                      ...
                      $('<article class="tile is-child">')
                        .append(
                          $('<img>')
                            .attr(
                              'src',
                              getAppealCard(currentProfile.appeal, currentProfile.version)
                            )
                            .css('width', '150px')
                        )
                    ...
                    $('<div class="tile is-parent is-6">').append(
                      $('<article class="tile is-child">').append(
                        $('<div>')
                          ...
                          .append($('<div>').append('Appeal Title:').css('font-size', '15'))
                          .append(
                            $('<div>')
                              .append(getAkaname(currentProfile['akaname']))
                              .css('font-size', '35px')
                          )
              ...
              $('<div class="tile is-ancestor">')
                    ...
                    $('<article class="tile is-child">').append(
                      $(
                        ...
                        .append(
                          ...
                          ).append($('<img id="skillFrame" style="height:100%">'))
                        )
                        .append(
                          $(
                ...
                .append(
                  $('<div class="tile is-parent is-5">').append(
                    $('<article class="tile is-child">').append(
                      $('<p class="title">')
                        .append(currentProfile.version === 6 ? 'PCB' : 'BLC / PC')
                        .append(
                          $('<div class="content">').append(
                            currentProfile.version === 6
                              ? currentProfile.blocks
                              : currentProfile.blocks + ' / ' + currentProfile.packets
                          )
                        )
                        .css('font-family', 'testfont')
                    )
      ...
      .append(
        $('<div class="card  is-inlineblocked">')
          .append(
            $('<div class="card-header">').append(
              $('<p class="card-header-title">')
                .append($('<span class="icon">').append($('<i class="mdi mdi-pulse">')))
                .append('Arena Stats')
            )
          )
          .append(
                    ...
                    $('<div class="tile is-parent">').append(
                      $('<div class="tile is-child field">').append(
                        $('<div class="control">').append(
                          $('<div class="select">').append($('<select id="arena-szn-sel">'))
                        )
                      )
                    )
                  )
                  ...
                  $('<ul class="is-marginless">')
                    .append(
                      $('<li class="is-active" name="cmpd">').append(
                        $('<a onclick="setCMpD()">').append('Clear Mark per Difficulty')
                      )
                    )
                    .append(
                      $('<li name="cmpl">').append(
                        $('<a onclick="setCMpL()">').append('Clear Mark per Level')
                      )
                    )
                    .append(
                      $('<li name="gpd">').append(
                        $('<a onclick="setGpD()">').append('Grade per Difficulty')
                      )
                    )
                    .append(
                      $('<li name="gpl">').append(
                    ...
                    .append(
                      $('<li name="aspl">').append(
                        $('<a onclick="setASpL()">').append('Average Score per Level')
                      )
                    )
                )
              )
    ...
    arena_data = arena_data
      .filter(are => are.version === currentVersion)
      .sort(function (a, b) {
    ...
    $('#skillTitle').attr(
      ...
      'font-size:25px; color:black; font-weight:bold;color:' +
        (getPlayerSkill()[0] === 11 ? '#FFC100' : getPlayerSkill()[0] === 12 ? '#FFE000' : 'black')
    );
