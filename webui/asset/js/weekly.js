function populateRankList(rankings) {
  ...
  $('#ranking').DataTable({
    data: rankings,
    columns: [{ data: 'rank' }, { data: 'name' }, { data: 'exscore' }, { data: 'date' }],
    columnDefs: [],
    responsive: {},
    paging: false,
    searching: false,
...
function populateWeeklySongsList(weekly, mdb) {
  ...
  for (let songCtr = weekly.length - 1; songCtr >= 0; songCtr--) {
    ...
    if (now < end || compltWeekCtr <= 3) {
      $('#wsongs tbody')
        .append('<tr>')
        .append('<td>' + songInfo.info.title_name + '</td>')
        .append('<td>' + startFormat + ' ~ ' + endFormat + '</td>')
        .append(
          '<td style="text-align:center;">' +
            (Number(now) > Number(weekly[songCtr]['start'])
              ? songInfo.difficulty.novice !== '0'
                ? '<img id="difRank" week=' +
                  weekly[songCtr]['weekId'] +
                  ' mid=' +
                  weekly[songCtr]['musicId'] +
                  ' mtype=0 src="static/asset/difficulty/level_small_nov.png">'
                : '-'
              : '-') +
            '</td>'
        )
        .append(
          '<td style="text-align:center;">' +
            (Number(now) > Number(weekly[songCtr]['start'])
              ? songInfo.difficulty.advanced !== '0'
                ? '<img id="difRank" week=' +
                  weekly[songCtr]['weekId'] +
                  ' mid=' +
                  weekly[songCtr]['musicId'] +
                  ' mtype=1 src="static/asset/difficulty/level_small_adv.png">'
                : '-'
              : '-') +
            '</td>'
        )
        .append(
          '<td style="text-align:center;">' +
            (Number(now) > Number(weekly[songCtr]['start'])
              ? songInfo.difficulty.exhaust !== '0'
                ? '<img id="difRank" week=' +
                  weekly[songCtr]['weekId'] +
                  ' mid=' +
                  weekly[songCtr]['musicId'] +
                  ' mtype=2 src="static/asset/difficulty/level_small_exh.png">'
                : '-'
              : '-') +
            '</td>'
        )
        .append(
          '<td style="text-align:center;">' +
            (Number(now) > Number(weekly[songCtr]['start'])
              ? songInfo.difficulty.maximum !== '0'
                ? '<img id="difRank" week=' +
                  weekly[songCtr]['weekId'] +
                  ' mid=' +
                  weekly[songCtr]['musicId'] +
                  ' mtype=4 src="static/asset/difficulty/level_small_mxm.png">'
                : '-'
              : '-') +
            '</td>'
        )
        .append(
          '<td style="text-align:center;">' +
            (Number(now) > Number(weekly[songCtr]['start'])
              ? songInfo.info.inf_ver !== '0'
                ? '<img id="difRank" week=' +
                  weekly[songCtr]['weekId'] +
                  ' mid=' +
                  weekly[songCtr]['musicId'] +
                  ' mtype=3 src="static/asset/difficulty/level_small_' +
                  infdiff[parseInt(songInfo.info.inf_ver) - 2] +
                  '.png">'
                : '-'
              : '-') +
            '</td>'
    ...
    try {
      ...
      if (xhr.status === 200) {
        ...
        if (custom && custom.mdb && custom.mdb.music)
          mdb.mdb.music = mdb.mdb.music.concat(custom.mdb.music);
      }
    } catch (e) {}
  ...
  $(document).on('click', '#difRank', async function () {
    ...
    await emit('getWeekRankList', { week: week, mid: mid, mtype: mtype, version: version }).then(
      function (response) {
        ...
        $('.songnamedif').remove();
        $('.song-info').append(
          "<h3 class='songnamedif'>" +
            mdb.mdb.music[songIndex]['info']['title_name'] +
            ' (' +
            (mtype === 3
              ? difLabels[mtype][parseInt(mdb.mdb.music[songIndex]['info']['inf_ver']) - 2]
              : difLabels[mtype]) +
            ')</h3>'
        );
      },
