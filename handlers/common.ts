...
import {
  ...
  LICENSED_SONGS7,
  CURRENT_ARENA7,
  ARENA_STATION_ITEMS7,
  VALGENE7,
  APIGENE7,
  INFORMATION7,
  UNLOCK_EVENTS7,
} from '../data/nbl';
import { getVersion, getRandomIntInclusive, loadMusicDb } from '../utils';
  ...
  try {
    ...
    switch (info.method) {
      case 'sv6_common': {
        ...
        if (IO.Exists('webui/asset/config/flags.json')) {
          ...
          for (const flagIter in flagConfig) {
            if (flagConfig[flagIter]['toggle']) {
              if (typeof flagConfig[flagIter]['str'] === 'string')
        ...
        information = INFORMATION6.filter(
          info => version >= info.version && currentYMDDate >= info.start
        );
        EXTENDS6.filter(ex => version >= ex.version && currentYMDDate >= ex.start).forEach(val =>
          extend.push(Object.assign({}, val))
        );
      ...
      case 'sv7_common': {
        ...
        information = INFORMATION7.filter(
          info => version >= info.version && currentYMDDate >= info.start
        );
        ...
        apigene = APIGENE7;
        EXTENDS7.filter(ex => version >= ex.version && currentYMDDate >= ex.start).forEach(val =>
          extend.push(Object.assign({}, val))
        );
        songNum = 2400;
        break;
      ...
      for (let i = 0; i <= songNum; i++) {
        ...
        if (foundSongIndex != -1) {
          ...
          if (Math.abs(gameVersion) === 6) {
            if (
              parseInt(songData.info.version) <= 6 &&
              'distribution_date' in songData['info'] &&
              parseInt(songData['info']['distribution_date']) > currentYMDDate
            ) {
            ...
            let crossResonance = [
              2231, 2232, 2233, 2260, 2261, 2262, 2284, 2285, 2286, 2339, 2340, 2341,
            ];
            if (
              parseInt(songData.info.version) <= 7 &&
              'distribution_date' in songData['info'] &&
              parseInt(songData['info']['distribution_date']) > currentYMDDate
            ) {
              console.log('Unreleased song: ' + songData.info.title_name);
          ...
          if (
            parseInt(songData.info.version) < Math.abs(gameVersion) &&
            licensedSongs.includes(i)
          ) {
            limitedNo += 1;
            for (let j = 0; j < 6; j++) {
              if (songData.difficulty[diffName[j]] != '0') {
    ...
    if (information.length > 0) {
      ...
      for (const info of information) {
        extend.push({
          id: info.id,
          type: 1,
          params: [1, currentTime, 0, 0, 0, '[f:0]SERVER INFORMATION', info.str, '', '', ''],
        });
      }
    }
    ...
    if (IO.Exists('webui/asset/config/events.json')) {
      ...
      for (const eData of eventData['events' + Math.abs(gameVersion)]) {
        ...
        if (stmpEvntInfo && version >= eData.version && currentYMDDate >= eData.start) {
          if (
            eData.type === 'stamp' &&
            eventConfig[eData.id] !== undefined &&
            eventConfig[eData.id].toggle
          ) {
            for (const stmpData of stmpEvntInfo.info.data) {
              extend.push({
                ...
                params: [
                  ...
                  'stmpHdJ' in stmpEvntInfo.info
                    ? stmpEvntInfo.info.stmpHdJ
                    : stmpEvntInfo.info.stmpHd,
                  stmpEvntInfo.info.stmpHd,
                  'stmpFtJ' in stmpEvntInfo.info
                    ? stmpEvntInfo.info.stmpFtJ
            ...
            extend.push({
              type: 19,
              id: stmpEvntInfo.info.id,
              params: [0, 0, 0, 0, 0, JSON.stringify(stmpEvntInfo.info.data), '', '', '', ''],
            });
          } else if (
            eData.type === 'tama' &&
            eventConfig[eData.id] !== undefined &&
            ...
            extend.push({
              type: 20,
              id: stmpEvntInfo.info.id,
              params: [0, 0, 0, 0, 0, stmpEvntInfo.info.list, '', '', '', ''],
            });
          } else if (
            eData.type === 'variant' &&
            eventConfig[eData.id] !== undefined &&
          ...
          eData.id === 'achmissions' &&
          version >= eData.version &&
          currentYMDDate >= eData.start
        ) {
    ...
    if (arenaOpen && version >= 20220425 && currentArena.season !== 0) {
      arenaData = {
        ...
        catalog:
          shopOpen &&
          U.GetConfig('arena_station') !== 'None' &&
          version >= arenaItems[U.GetConfig('arena_station')].version
            ? arenaItems[U.GetConfig('arena_station')].items.map(item => ({
    ...
    valgene_info = valgene.info
      .filter(val => version >= val.version)
      .map(val => ({
    ...
    if (
      ['10/24/', '10/25/', '10/26/', '10/27/', '10/28/', '10/29/', '10/30/', '10/31/'].includes(
        currentDate.substring(0, 6)
      )
    )
    ...
    if (IO.Exists('webui/asset/config/weeklymusic.json')) {
      ...
      for (let weekIter in weeklyMusic) {
        if (Number(date) > weeklyMusic[weekIter].start && Number(date) <= weeklyMusic[weekIter].end)
          weekData = weeklyMusic[weekIter];
      }
      ...
      skill_course: {
        info: courses.reduce((acc, s) => {
          if (version >= s.version) {
            ...
            if (
              (info.model.split(':')[2] === 'G' || info.model.split(':')[2] === 'H') &&
              s.hasGod !== undefined &&
              s.hasGod === 1 &&
              version >= 20230530
            ) {
    ...
    send.object(response, { encoding: 'utf8' });
  } catch (error) {
    console.log(error);
  }
};
